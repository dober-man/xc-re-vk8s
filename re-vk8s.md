## RE vK8s
This simplified diagram shows how globally distributed users access resources closest to their region. In reality there are dozens of XC PoPs all across the planet and due to how anycast works, users will automatically route to the closest destination to their source. The vK8s namespace can be replicated across all the PoPs or selectively, making the pods globally redundant and available closest to the source. 
